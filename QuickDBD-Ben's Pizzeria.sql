...
CREATE INDEX idx_recipe_id ON `item` (`sku`);
...
CREATE INDEX idx_inventory_id ON `recipe` (`ing_id`);
...
CREATE INDEX idx_ingredient_id ON `recipe` (`ing_id`);
...
CREATE INDEX idx_rota_id ON `orders` (`created_at`);
CREATE INDEX idx_rota_id ON `staff` (`staff_id`);
...
ALTER TABLE `customers` ADD CONSTRAINT `fk_customer_cust_id` FOREIGN KEY(`cust_id`)
...
ALTER TABLE `ingredient` ADD CONSTRAINT `fk_ingredient_ing_name` FOREIGN KEY(`ing_id`)
REFERENCES `recipe` (`ing_id`);
